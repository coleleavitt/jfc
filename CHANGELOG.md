...
- **Graph content index (persistent `graph_grep` cache)**: new `content_index.rs` — a `DashMap`-backed, mtime-validated line cache + revision-gated per-file symbol-span index. Eliminates the two perf gaps in `graph_grep`: (1) files are read once and reused across calls instead of re-read every invocation; (2) enclosing-symbol lookup is now a binary search over a cached, start-sorted span list instead of an O(M×N) scan of every graph node per match. Invalidated per-file on `file_changed`. Measured 54ms→10ms (5.5×) on a warm repeat over the 15k-node tree; `graph_status` now reports warm-file count. +3 tests
