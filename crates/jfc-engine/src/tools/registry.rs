...
pub fn graph_session_cache() -> &'static std::sync::Mutex<
...
pub fn graph_session_cache_key(cwd: &std::path::Path) -> std::path::PathBuf {
    cwd.canonicalize().unwrap_or_else(|_| cwd.to_path_buf())
}

pub fn build_graph_session_for_key(
...
/// `&mut GraphSession`.
pub fn with_graph_session_mut<R>(
    cwd: &std::path::Path,
...
pub fn get_or_build_graph_session(
    cwd: &std::path::Path,
) -> Arc<jfc_graph::session::GraphSession> {
    let key = graph_session_cache_key(cwd);
...
pub fn snapshot_active_provider()
...
pub fn graph_history() -> &'static std::sync::Mutex<jfc_graph::history::GraphHistory> {
...
pub fn record_graph_query(query: &str, result: &jfc_graph::dsl::QueryResult) {
...
pub fn auto_context_queue() -> &'static std::sync::Mutex<Vec<std::path::PathBuf>> {
...
/// boundary.
pub fn record_edited_file(path: &std::path::Path) {
