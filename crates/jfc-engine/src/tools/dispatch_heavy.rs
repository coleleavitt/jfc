...
pub fn execute_graph_outline(file: String, cwd: &Path) -> ExecutionResult {
    let session = get_or_build_graph_session(cwd);
...
pub fn execute_graph_status(cwd: &Path) -> ExecutionResult {
    let session = get_or_build_graph_session(cwd);
...
pub fn execute_graph_files(path_filter: Option<&str>, cwd: &Path) -> ExecutionResult {
    let session = get_or_build_graph_session(cwd);
    let graph = &session.graph;
...
pub fn execute_get_program_slice(
    symbol: String,
    backward: bool,
    max_nodes: Option<usize>,
    cwd: &Path,
...
pub fn execute_get_data_dependencies(
    symbol: String,
    max_nodes: Option<usize>,
    cwd: &Path,
) -> ExecutionResult {
...
pub async fn execute_symbol_edit(
    handle: String,
    new_content: String,
    validate: bool,
