...
pub fn execute_push_notification(message: &str, title: Option<&str>) -> ExecutionResult {
    if message.is_empty() {
...
// ─── RemoteTrigger tool ────────────────────────────────────────────────────
pub async fn execute_remote_trigger(
    trigger_id: &str,
...
pub fn remote_trigger_config_path() -> Option<std::path::PathBuf> {
...
pub fn parse_trigger_url(toml_text: &str, trigger_id: &str) -> Result<String, String> {
