...
pub fn slash_spec(name: &'static str, help: &'static str) -> StaticSpec {
    StaticSpec {
        name,
        description: help,
...
pub fn tool_spec(kind: crate::types::ToolKind) -> StaticSpec {
...
pub fn tool_is_mutating(kind: crate::types::ToolKind) -> bool {
...
pub fn register_slash_commands(table: &'static [(&'static str, &'static str)]) {
...
pub fn slash_help_lines() -> String {
...
pub fn all_specs() -> Vec<StaticSpec> {
...
pub fn render_help(specs: &[&dyn CommandSpec]) -> String {
    if specs.is_empty() {
        return String::new();
    }
    let mut out = String::new();
...
pub fn render_manifest(specs: &[&dyn CommandSpec]) -> String {
    let mut out = String::new();
    for spec in specs {
        out.push_str(&format!(
...
pub fn render_manifest_all() -> String {
    let specs = all_specs();
    let refs: Vec<&dyn CommandSpec> = specs.iter().map(|s| s as &dyn CommandSpec).collect();
...
pub fn render_completions(specs: &[&dyn CommandSpec]) -> String {
