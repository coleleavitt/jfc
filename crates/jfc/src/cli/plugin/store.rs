...
pub(crate) use crate::plugin_paths::{plugins_root, sanitize_plugin_name};
...
pub(super) fn update_plugins(name: Option<&str>) -> anyhow::Result<Vec<String>> {
...
pub(super) fn remove_plugin(name: &str) -> anyhow::Result<PathBuf> {
    let name = sanitize_plugin_name(name)?;
...
pub(super) fn prepare_dest(dest: &Path, force: bool) -> anyhow::Result<()> {
    if !dest.exists() {
        return Ok(());
    }
    if !force {
...
pub(super) fn workflow_dir_for_plugin(path: &Path) -> Option<PathBuf> {
...
pub(super) fn looks_like_git_url(source: &str) -> bool {
