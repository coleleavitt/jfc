...
fn reasoning_header_uses_text_label_in_screen_reader_mode() {
    ...
    std::env::set_var("JFC_SCREEN_READER", "1");

    let t = Theme::dark();
    let mut items: Vec<super::core::RenderItem<'_>> = Vec::new();
    // Expanded=true path exercises the header rendering directly.
    super::assistant_parts::push_reasoning_lines(&mut items, "internal thoughts", true, &t);
    ...
    assert!(!header.contains("∴"), "should avoid decorative reasoning glyph in screen-reader mode");
}
