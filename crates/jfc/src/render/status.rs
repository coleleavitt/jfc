...
use ui_model::status::{StatusSegment as StatusSegmentModel, StatusTone};
...
pub(super) fn status(f: &mut Frame, app: &App, area: Rect) {
    ...
    for segment in crate::view_model::status::status_row(app).segments {
        segs.push(render_status_segment(segment, t));
    }
    if super::status_plugins::has_status_line_slot(
        &app.plugins.ui_slots,
...
fn render_status_segment(segment: StatusSegmentModel, theme: crate::theme::Theme) -> StatusSeg {
    StatusSeg {
        spans: vec![Span::styled(
            segment.text,
            status_tone_style(segment.tone, theme),
        )],
        prio: segment.priority,
    }
}

fn status_tone_style(tone: StatusTone, theme: crate::theme::Theme) -> Style {
    match tone {
        StatusTone::Muted => Style::default().fg(theme.text_muted),
        StatusTone::Alert => Style::default()
            .fg(theme.warning)
            .add_modifier(Modifier::BOLD),
        StatusTone::Error => Style::default().fg(theme.error),
        StatusTone::Activity => Style::default().fg(theme.accent_secondary),
        StatusTone::Accent => Style::default().fg(theme.accent),
        StatusTone::Success => Style::default().fg(theme.success),
        StatusTone::ShellActivity => Style::default()
            .fg(theme.bg)
...
mod tests {
    ...
    fn shell_activity_status_segment_preserves_rendered_badge_contract_normal() {
        let segment = StatusSegmentModel::new(" 1 shell ", StatusTone::ShellActivity, 87);
        assert_eq!(segment.text, " 1 shell ");
        assert_eq!(segment.tone, StatusTone::ShellActivity);
        assert_eq!(segment.priority, 87);

        let rendered = render_status_segment(segment, crate::theme::Theme::claude());
        assert_eq!(rendered.prio, 87);
        assert_eq!(rendered.spans[0].content.as_ref(), " 1 shell ");
    }
}
