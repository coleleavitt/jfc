...
//!
//! Ported verbatim from `openai/codex` `codex-rs/tui/src/streaming/chunking.rs`
//! (pure `std::time` — no `ratatui`/protocol deps; the only edit is dropping the
//! `pretty_assertions` test import, which JFC does not vendor). This is the
//! "feels alive, never lags" pacing engine: it drains one line per commit tick
//! while smooth, and batch-drains the backlog under burst load with hysteresis
//! so it never gear-flaps.
//!
//! # Mental model
//!
