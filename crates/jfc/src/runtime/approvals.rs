...
pub(crate) fn insert_tool_into_message(_state: &mut EngineState, _tool: &ToolCall) {
...
pub(crate) fn dispatch_approved_tools(state: &mut EngineState, tools: Vec<ToolCall>, tx: &mpsc::Sender<EngineEvent>) {
    if tools.is_empty() {
        return;
    }
...
pub(crate) fn advance_approval_queue(state: &mut EngineState) -> Vec<ToolCall> {
...
pub(crate) fn finish_approval_decision(
...
pub(crate) fn deny_tool(state: &mut EngineState, tool: ToolCall) {
